...
%   \input{00-common-preamble.tex}
  ...
  \begin{document}
  \title{Learning Propagation Redundant SAT clauses via Conditional Autarkies}
  
  \author{TODO: add authors and orcid \\
%   \author{Yi Zhou \orcid{0000-0001-7597-1176}, Jay Bosamiya \orcid{0000-0002-5596-6828}, Jessica Li, Marijn J.\ H.\ Heule \orcid{0000-0002-5587-8801}, Bryan Parno \orcid{0000-0002-9113-1684}\\
  \emph{Carnegie Mellon University, Pittsburgh, PA, USA}\\
  \texttt{\{email1,email2,email3,email4\}@cmu.edu}}
  
  \maketitle
  
  \begin{abstract}
    % \input{texs/abstract}
    Propagation redundant (\pr) clauses are a class of satisfiability-preserving clauses that can be learned by modern SAT solvers.  Crucially, these techniques admit short proofs for important benchmark families, such as the pigeonhole principle and the mutilated chessboard. However, existing \pr clause learning techniques require calling a second SAT solver. Hence they are computationally expensive and difficult to integrate into existing tools.

    We propose a new method for learning \pr clauses based on conditional autarkies. Our method is easy to implement and learns \pr clauses in linear time. Additionally, we introduce a number of heuristics including a clause-shrinking technique to ensure that the learned clauses are useful. We show that it is competitive with state-of-the-art \pr clause learning techniques on a suite of SAT instances.
  ...
  \input{texs/1.introduction}
  \input{texs/2.background}
  \input{texs/3.clauseaddition.tex}
  \input{texs/4.optimizations}
  \input{texs/5.motivatingexample}
  \input{texs/6.implementation}
  \input{texs/7.evaluation}
