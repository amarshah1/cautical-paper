...
% However, this is not very useful as it rules out
...
% For literals $l_1$ and $l_2$, we sometimes notate $l_1 \impunit^\formula l_2$
% to mean $\formula \vdash_1 \overline{l}_1 \land l_2$. Informally, we can think
% of this as saying ``$l_1$ implies $l_2$ via unit propagation on $\formula$,''
% which means that . see definition here:
...
In plain words, an autarky is an assignment that satisfies every clause it
...
% todo we use O(n^_) where n is formula, but also where n is the number of
% variables.
