...
extend  an assignment $\alpha$ to literals, by denoting $\alpha(l) = \alpha(x)$
...
mapping each $l_i \in C$ to false ($\bot$). An assignment $\alpha$ \emph{touches}
...
without hints is NP-complete~\cite{prclauses}. Thus, the propagation redundancy (\pr)
proof system introduces witnesses, which must be provided for proof checking.  
...
touches. Unfortunately, formulas often do not contain autarkies, and they are difficult
to find even when they exist. Instead, we use the following weakening of an autarky:
