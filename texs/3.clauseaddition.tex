...
For any $n$, $\ph{n}$ is clearly unsatisfiable. However, there are no polynomial-sized resolution proofs of \ph{n}~\cite{hakenpigeonhole}. Our methodology obtains an $O(n^3)$ \pr proof of \ph{n} matching the best know result~\cite{prclauses}. Additionally, we provide a proof with very small constant factor and our method is insensitive to the encoding.
...
Specifically, the greedy algorithm will find the largest set in $\{\alpha_a^{SETS} \cap (\alpha_c \backslash C_0) | a \in \alpha_a \}$ at each step and add it to $C_0$. It stops when each of the $\alpha_a^{SETS} \cap (\alpha_c \backslash C_0)$ are empty.
