...
\autoref{alg:leastcond} provides $\alpha_c \sqcup \alpha_a$, which is a conditional autarky, as every
...
\begin{proof}
    ...
    % We have that Γ is satisfiable =>  Γ ∧ (V c v a) is satisfiable =>  Γ ∧ (V c v V a) is satisfiable =>  Γ is satisfiable. The first implication follows from Theorem 1. The second implication follows from the fact that we are weakening a clause. The third implication follows from the fact that we are removing a clause. We will clarify this in the paper.
...
\begin{algorithm}
    ...
    \Fn{\Shrink{$\formula$, $\alpha_a$, $\alpha_c$}}{
        ...
        ${\it SETS}$ := init \texttt{Array[len($\alpha_a$)]}\;
