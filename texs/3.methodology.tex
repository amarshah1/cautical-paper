...
factor and robustness against encoding perturbations.
...
At a high level, \autoref{alg:methodology} learns a \pr clause by: (1)
propagating on two variables $i$ and $j$, (2) learning the least conditional
autarky of the partial assignment, (3) shrinking the clause, and (4) adding
the clause to the formula. 

The \texttt{Propagate} function represents unit propagation.
\texttt{LeastConditional} function achieves 2, following prior work
(\autoref{subsec:learning}). The \texttt{Shrink} function takes care of 3
(\autoref{subsec:shrinking}). All four stages require several heuristic choices (\autoref{sec:implementation}).

\begin{algorithm} \caption{Learning \pr clauses}
    \SetAlgoNoLine
...
In a formula $\formula$ with conditional autarky $\alpha = \alpha_c \sqcup \alpha_a$ and $\alpha_c = c_1, \dots, c_n$ and $\alpha_a =
...
As mentioned in \autoref{subsec:shrinking}, our choice for $A_0$ is important. We discuss our heuristic for minimizing the size of $A_0$ without drastically increasing complexity.
